...
import { ArrowLeft, Bell, Settings, Menu, History, Wallet, BarChart2 } from "lucide-react";
        ...
        <div className="min-h-screen bg-slate-950 text-slate-50 flex flex-col">
            ...
            <header className="h-16 border-b border-slate-800 flex items-center justify-between px-6 bg-slate-950/50 backdrop-blur-md sticky top-0 z-50">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button className="p-2 text-slate-400 hover:text-white transition-colors md:hidden" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
                        <Menu className="w-5 h-5" />
                    </button>
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-primary to-emerald-600 border border-white/20 hidden md:block"></div>
                </div>
            </header>
            ...
            <main className="flex-1 p-4 grid grid-cols-1 lg:grid-cols-12 gap-4 overflow-hidden h-[calc(100vh-64px)] relative">
                ...
                {mode => activeTab === 'trade' ? (
                    <>
                        {/* Left Sidebar (Stocks) - Hidden on Mobile unless menu open? Keep hidden for now or using drawers */}
                        <div className={cn(
                            "lg:block lg:col-span-3 xl:col-span-2 h-full overflow-hidden transition-all duration-300 z-40",
                            mobileMenuOpen ? "absolute inset-0 bg-slate-950 p-4 block" : "hidden"
                        )}>
                            <div className="h-full flex flex-col gap-4">
                                <div className="lg:hidden flex justify-end">
                                    <button onClick={() => setMobileMenuOpen(false)} className="p-2 text-white"><X className="w-6 h-6" /></button>
                                </div>
                                <StockList />
                ...
                <div className="lg:hidden fixed bottom-0 left-0 right-0 p-4 bg-slate-950 border-t border-slate-800 z-30">
            ...
            <div className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-slate-900 border-t border-slate-800 flex items-center justify-around z-50">
                <button onClick={() => setActiveTab('trade')} className={cn("flex flex-col items-center gap-1", activeTab === 'trade' ? "text-primary" : "text-slate-500")}>
                    <BarChart2 className="w-5 h-5" />
                    <span className="text-[10px]">Trade</span>
                </button>
                <button onClick={() => setActiveTab('history')} className={cn("flex flex-col items-center gap-1", activeTab === 'history' ? "text-primary" : "text-slate-500")}>
...
// Importing X icon strictly for use inside the component function
